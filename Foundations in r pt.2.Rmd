...
colnames(penguins)[3]<- "billLengthMm"
colnames(penguins)
colnames(penguins)[4:6]<- c("billDepthMn", "flipperLengthMn", "bodyMassG")
```
...
```{r}
penguins$billInfo<- penguins$billLengthMm+ penguins$billDepthMn
library(tidyverse)
penguins %>% select(billLengthMm, billDepthMn, billInfo)
```
...
```{r}
tolower(colnames(mtcars))[4] #doesn't save it
...
for(i in 1:length(vectNum)){
  number[i]<-vectNum[i]*4
}
for(placeholder in 1:length(vectNum)){
  print(vectNum[placeholder]*4)
}
...
for(i in 1:nrow(mtcars)){
  if(mtcars$mpg[i]>25){
    mtcars$above25<- "good milage"
   } else{mtcars$above25<-"bad milage"}
  }
```
...
ifelse(mtcars$mpg>25, "good milage", "bad milage")
```
...
for(i in 1:nrow(ChickWeight)){
  if(ChickWeight$weight[i]>100){
    ChickWeight$above100[i]<- "big chick"
  }
}
```
for(){
